...
def test_query_soilgrids():
    ...
    assert resp['geometry']['coordinates'] == [60, 50], "Response should have be for lat=50, lon=60"
    
    returned_properties = [l['name'] for l in resp['properties']['layers']]
    assert set(returned_properties) == {'clay', 'ocs'}, "Response should include properties clay and ocs"
    
    returned_depths = [d['label'] for l in resp['properties']['layers'] for d in l['depths']]
    assert set(returned_depths) == {'0-5cm', '0-30cm'}, "Response should include depths 0-5cm and 0-30cm"
    
    returned_values = [v for l in resp['properties']['layers'] for d in l['depths'] for v in d['values']]
