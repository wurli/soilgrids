...
def test_parse_depth():
    ...
    assert 'depth'        in ocs.keys(), "Parsed depth should have a 'depth' column"
    assert 'top_depth'    in ocs.keys(), "Parsed depth should have a 'top_depth' column"
    assert 'bottom_depth' in ocs.keys(), "Parsed depth should have a 'bottom_depth' column"
    assert 'unit_depth'   in ocs.keys(), "Parsed depth should have a 'unit_depth' column"
    assert 'mean'         in ocs.keys(), "Parsed depth should have a 'mean' column"
    assert 'uncertainty'  in ocs.keys(), "Parsed depth should have a 'uncertainty' column"
...
def test_parse_property():
    ...
    assert 'soil_property'    in clay.keys(), "Parsed property should have a 'soil_property' column"
    assert 'd_factor'         in clay.keys(), "Parsed property should have a 'd_factor' column"
    assert 'mapped_units'     in clay.keys(), "Parsed property should have a 'mapped_units' column"
    assert 'target_units'     in clay.keys(), "Parsed property should have a 'target_units' column"
    assert 'uncertainty_unit' in clay.keys(), "Parsed property should have a 'uncertainty_unit' column"
    assert 'depth'            in clay.keys(), "Parsed property should have a 'depth' column"
    assert 'top_depth'        in clay.keys(), "Parsed property should have a 'top_depth' column"
    assert 'bottom_depth'     in clay.keys(), "Parsed property should have a 'bottom_depth' column"
    assert 'unit_depth'       in clay.keys(), "Parsed property should have a 'unit_depth' column"
    assert 'mean'             in clay.keys(), "Parsed property should have a 'mean' column"
    assert 'uncertainty'      in clay.keys(), "Parsed property should have a 'uncertainty' column"
